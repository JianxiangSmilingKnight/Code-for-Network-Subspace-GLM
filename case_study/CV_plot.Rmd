...
for (II in 1:h) {
  # Get the indices where `cv_order` matches the current fold
  fold_indices <- which(cv_order == II)
  
  # Filter those indices where `tmp` matches elements in `school_choice[1,]`
  matching_indices <- fold_indices[tmp[fold_indices] %in% school_choice[II,]]

  # Append the matching indices to the result vector
  selected_indices <- c(selected_indices, matching_indices)
}

true_labels <- Y[selected_indices]
if(sum(true_labels)>0){
 predicted_probs_method1 <- prediction[selected_indices]
predicted_probs_method2 <- prediction_NL[selected_indices]
predicted_probs_method3 <- prediction_RNC[selected_indices]
...
legend("bottomright", legend = c("Our Model:0.711", "RNC:0.694", "Logistic Reg:0.696"), col= c("navy","magenta","green"))
}
...
plot(roc_curve_method2,col="green", add = TRUE)
plot(roc_curve_method3,col="magenta", add = TRUE)
legend("bottomright", legend = c("Our Model:0.722", "RNC:0.704", "Logistic Reg:0.710"), col= c("navy","magenta","green"))
}
...
plot(roc_curve_method2,col="green", add = TRUE)
plot(roc_curve_method3,col="magenta", add = TRUE)
legend("bottomright", legend = c("Our Model:0.706", "RNC:0.733", "Logistic Reg:0.701"), col= c("navy","magenta","green"),lty = 1)
}
roc_curve_method1$auc
roc_curve_method2$auc
roc_curve_method3$auc
