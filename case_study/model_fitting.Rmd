...
for (II in 1:h) {
  ...
  full_model<-SP.back(perm.X[1:(n-M),],perm.Y[1:(n-M)],perm.A.orig[1:(n-M),1:(n-M)],usvt.reg$Km,0.95,0.05,keep.index=c(7))
  X_SP<-X.true[, colnames(X.true) %in% colnames(full_model$X)]
  SP.perm <- SP.semi.Inf(X=X_SP,Y = matrix(perm.Y[1:(n-M)],ncol=1),A=perm.A.orig,K = usvt.reg$Km,r=0,thr=0.95)
