# Subspace Poisson regression for network embedding
...
## Load the network embedding results
...
## We adjust the eigenvectors direction so that for network with different density, the data scale (for X\beta, X\theta, \alpha) remains the same.
## A different similarity matrix will need to adjust the directions again.
## Compared to Logistic Reg, Poisson Reg is more sensible to data scale
## The general principle to adjust the eigenvectors direction is to keep the range and the mean of EY small
...
estimate_2000_2logn<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(matrix_list[[j]])
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
  ...
  tmp<-t(tmp)
  estimate_2000_2logn[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
...
alpha <- W%*%alpha.coef
EY <-exp(Xtheta+Xbeta+alpha)
estimate_2000_n12<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_n12[[j]])%*%t(as.matrix(record_2000_n12[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
  ...
  tmp<-t(tmp)
  estimate_2000_n12[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
...
alpha <- W%*%alpha.coef
EY <-exp(Xtheta+Xbeta+alpha)
estimate_2000_n23<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_n23[[j]])%*%t(as.matrix(record_2000_n23[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
  ...
  tmp<-t(tmp)
  estimate_2000_n23[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
...
estimate_2000_2logn<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_2logn[[j]])%*%t(as.matrix(record_2000_2logn[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
...
alpha <- W%*%alpha.coef
EY <-exp(Xtheta+Xbeta+alpha)
estimate_2000_Diff2Vec_2logn<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_Diff2Vec_2logn[[j]])%*%t(as.matrix(record_2000_Diff2Vec_2logn[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
  ...
  tmp<-t(tmp)
  estimate_2000_Diff2Vec_2logn[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
...
alpha <- W%*%alpha.coef
EY <-exp(Xtheta+Xbeta+alpha)
estimate_2000_Diff2Vec_n12<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_Diff2Vec_n12[[j]])%*%t(as.matrix(record_2000_Diff2Vec_n12[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
  ...
  tmp<-t(tmp)
  estimate_2000_Diff2Vec_n12[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
...
alpha <- W%*%alpha.coef
EY <-exp(Xtheta+Xbeta+alpha)
estimate_2000_Diff2Vec_n23<-matrix(0,nrow = B,ncol = 6)
for (j in c(1:B)) {
  A<-as.matrix(record_2000_Diff2Vec_n23[[j]])%*%t(as.matrix(record_2000_Diff2Vec_n23[[j]]))
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    Y <- rpois(n,EY)
    fit <- SP.Inf.Poisson(X.true,Y,A,K=3,r=1,boot.thr=FALSE)
    bias=fit$beta[2,]-Xrho
  ...
  tmp<-t(tmp)
  estimate_2000_Diff2Vec_n23[j,]<-c(mean(tmp[,1]),sd(tmp[,2]),mean(tmp[,1]^2),mean(tmp[,2]),mean(tmp[,3]),mean(tmp[,4]))
}
