...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="glmnet",.combine = cbind) %dopar% {
    ...
    fit_perm <- glmnet(Xtmp, Y, family = "binomial", alpha = 0,intercept = FALSE, standardize=FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
...
save(record_500_2logn,file="Diag_L_record_500_2logn.Rda")
...
save(record_500_n1.2,file="Diag_L_record_500_n1.2.Rda")
...
save(record_500_n2.3,file="Diag_L_record_500_n2.3.Rda")
...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="glmnet",.combine = cbind) %dopar% {
    ...
    Xtmp<-cbind(X.true,(eigen_decomp$vectors))
    penalty_factors <- c(rep(0, 2), eigen_decomp$values)
    cv_fit_perm <- cv.glmnet(Xtmp, Y,intercept = FALSE, standardize=FALSE, family = "binomial",nfolds = 10, penalty.factor = penalty_factors,lambda =seq(2,1,length.out=10), alpha = 0)
    optimal_lambda_perm <- cv_fit_perm$lambda.min
    fit_perm <- glmnet(Xtmp, Y, family = "binomial", alpha = 0,intercept = FALSE, standardize=FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
    fitted_value<-exp(Xtmp%*%fit_perm$beta)/(1+exp(Xtmp%*%fit_perm$beta))
    SPE<-as.matrix(t(fitted_value-EY)%*%(fitted_value-EY)/n)
...
save(record_1000_2logn,file="Diag_L_record_1000_2logn.Rda")
...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    ...
    Xtmp<-cbind(X.true,eigen_decomp$vectors)
    penalty_factors <- c(rep(0, 2), eigen_decomp$values)
    cv_fit_perm <- cv.glmnet(Xtmp, Y,intercept = FALSE, standardize=FALSE, family = "binomial",nfolds = 10, penalty.factor = penalty_factors, alpha = 0,lambda = seq(2,1,length.out=10))
    optimal_lambda_perm <- cv_fit_perm$lambda.min
    fit_perm <- glmnet(Xtmp, Y,  family = "binomial", alpha = 0,intercept = FALSE, standardize=FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
    fitted_value<-exp(Xtmp%*%fit_perm$beta)/(1+exp(Xtmp%*%fit_perm$beta))
    SPE<-as.matrix(t(fitted_value-EY)%*%(fitted_value-EY)/n)
...
save(record_1000_n1.2,file="Diag_L_record_1000_n1.2.Rda")
...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    ...
    Xtmp<-cbind(X.true,(eigen_decomp$vectors))
    penalty_factors <- c(rep(0, 2), eigen_decomp$values)
    cv_fit_perm <- cv.glmnet(Xtmp, Y, family = "binomial",intercept = FALSE, standardize=FALSE,nfolds = 10, penalty.factor = penalty_factors, alpha = 0,lambda = seq(2,1,length.out=10))
    optimal_lambda_perm <- cv_fit_perm$lambda.min
    fit_perm <- glmnet(Xtmp, Y, family = "binomial", alpha = 0,intercept = FALSE, standardize=FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
    fitted_value<-exp(Xtmp%*%fit_perm$beta)/(1+exp(Xtmp%*%fit_perm$beta))
    SPE<-as.matrix(t(fitted_value-EY)%*%(fitted_value-EY)/n)
...
save(record_1000_n2.3,file="Diag_L_record_1000_n2.3.Rda")
