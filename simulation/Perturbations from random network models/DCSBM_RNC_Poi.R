...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="glmnet",.combine = cbind) %dopar% {
    ...
    cv_fit_perm <- cv.glmnet(Xtmp, Y, family = "poisson",nfolds = 10, penalty.factor = penalty_factors,lambda = seq(2,1,length.out=10), alpha = 0)
    optimal_lambda_perm <- cv_fit_perm$lambda.min
    fit_perm <- glmnet(Xtmp, Y, family = "poisson",thresh = 1e-03, alpha = 0,intercept = FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
...
for (i in c(1:B)) {
  ...
  tmp<-foreach(i=1:M,.packages="RSpectra",.combine = cbind) %dopar% {
    ...
    Xtmp<-cbind(X.true,eigen_decomp$vectors)
    penalty_factors <- c(rep(0, 2), eigen_decomp$values)
    cv_fit_perm <- cv.glmnet(Xtmp, Y, family = "poisson",nfolds = 10, penalty.factor = penalty_factors, alpha = 0,lambda = seq(2,1,length.out=10))
    optimal_lambda_perm <- cv_fit_perm$lambda.min
    fit_perm <- glmnet(Xtmp, Y,  family = "poisson",thresh = 1e-03, alpha = 0,intercept = FALSE, penalty.factor =penalty_factors,lambda = optimal_lambda_perm)
    fitted_value<-exp(Xtmp%*%fit_perm$beta)
    SPE<-as.matrix(t(fitted_value-EY)%*%(fitted_value-EY)/n)
